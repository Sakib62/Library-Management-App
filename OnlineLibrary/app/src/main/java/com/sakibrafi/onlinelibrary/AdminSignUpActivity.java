...
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseReference = FirebaseDatabase.getInstance().getReference("Librarians");
        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                ...
                if(task.getException() instanceof FirebaseAuthUserCollisionException) {
                    if(user.isEmailVerified()) {
                        Toast.makeText(AdminSignUpActivity.this, "Email is already used!", Toast.LENGTH_LONG).show();
                    }
                    else {
                        Toast.makeText(AdminSignUpActivity.this, "Verification email has already been sent. Click the link in the email to continue.", Toast.LENGTH_LONG).show();
                    }
                }
                else if (task.isSuccessful()) {
                    String key = databaseReference.push().getKey();
                    UserModel librarian = new UserModel(username, email, password);
                    databaseReference.child(key).setValue(librarian);

                    user.sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if(task.isSuccessful()) {
                                Toast.makeText(AdminSignUpActivity.this, "An email has been sent to your given address. Please click the link in the email to continue.", Toast.LENGTH_LONG).show();
                                startActivity(new Intent(AdminSignUpActivity.this, SignInActivity.class));
                                finish();
                            }
                            else {
                                Toast.makeText(AdminSignUpActivity.this, task.getException().getMessage(), Toast.LENGTH_LONG).show();
                            }
                        }
                    });
                }
                else { // If sign up fails, display a message to the user.
                    Toast.makeText(AdminSignUpActivity.this, "Authentication Failed.", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
